...
- Modify: `tests/test_update_check_ui.py`
...
- [ ] **Step 3: Add UI source guards** asserting the live-main banner hides mutation controls and tells the user to ask an agent to merge.
- [ ] **Step 4: Run the UI-focused tests** and confirm the new assertions fail for the current banner behavior.
...
- [ ] **Step 4: Run the new policy tests** and confirm they pass while existing non-live update behavior remains covered.
- [ ] **Step 5: Run the existing update suite** with `./scripts/test.sh tests/test_updates.py tests/test_update_checker.py tests/test_update_stash_recovery.py -q`; update obsolete live-main expectations only, never weaken the no-destruction assertions.

### Task 3: Make the update banner read-only in live-main mode
...
### Task 4: Point the actual LaunchAgent at live `main`
...
- [ ] **Step 1: Preserve a read-only copy and record the current LaunchAgent state** (`launchctl print`, plist contents, PID, listener, and current source path).
...
- [ ] **Step 4: Prove the running argv/source path**, `/health`, one deep health request, port ownership, and unchanged intentional worktree edits.

### Task 5: Install durable agent instructions
