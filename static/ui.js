...
function _compressionRecoverySourceHtml(session){
  const sourceSid=String(
    session&&session.compression_recovery_source_session_id||''
  ).trim();
  if(!sourceSid) return '';
  const icon=(typeof li==='function')?li('history',14):'';
...
async function openCompressionRecoverySource(btn){
  const sourceSid=String(
    btn&&btn.dataset&&btn.dataset.recoverySourceSessionId||''
  ).trim();
  if(!sourceSid||typeof loadSession!=='function') return false;
  try{
    await loadSession(sourceSid,{preserveActiveInput:false});
    if(typeof renderSessionList==='function') await renderSessionList();
    if(typeof _setActiveSessionUrl==='function') _setActiveSessionUrl(sourceSid);
    return true;
  }catch(e){
    if(typeof showToast==='function') showToast('Could not open source history: '+(e&&e.message||e),5000,'error');
    return false;
  }
}
...
function _findModelInDropdown(modelId, sel, preferredProviderId){
  ...
  // An explicit provider-qualified value is authoritative. If that provider
  // has no matching catalog row, do not fall through to a same-named model
  // owned by another provider; the caller will append the qualified value so
  // its route survives rehydration (#6221).
  if(explicitProvider) return null;
  // 2. Normalized match — only when unambiguous. A bare id shared by multiple
  // provider groups must not snap to the first DOM option when no provider
  // hint is available (#6195).
  const exact=opts.find(o=>norm(o)===target);
  if(exact){
    const normMatches=options.filter(o=>norm(o.value)===target);
    if(normMatches.length>1&&!preferred&&!explicitProvider&&!rawModel.includes('/')) return null;
    return exact;
  }
...
function _applyModelToDropdown(modelId, sel, preferredProviderId, opts){
  ...
  if(resolved){
    ...
    if(preferredProvider&&sel.options){
      // Assigning select.value chooses the first duplicate value; restore the
...
// Remember the dropdown's original footer slot so the phone path can move it
// to <body> without losing the desktop DOM position.
...
function _restoreModelDropdownHome(){
  ...
  if(_modelDropdownHome&&_modelDropdownHome.parent&&dd.parentNode!==_modelDropdownHome.parent){
    ...
    if(ref&&ref.parentNode===_modelDropdownHome.parent) _modelDropdownHome.parent.insertBefore(dd,ref);
    else _modelDropdownHome.parent.appendChild(dd);
  }
}
...
function _positionModelDropdown(){
  ...
  if(isPhone){
    if(!_modelDropdownHome) _modelDropdownHome={parent:dd.parentNode,nextSibling:dd.nextSibling};
    ...
    const titlebarBottom=titlebar&&typeof titlebar.getBoundingClientRect==='function'
      ? Number(titlebar.getBoundingClientRect().bottom)||0 : 0;
...
function renderModelDropdown(){
  ...
  const _filterModels=(term)=>{
    ...
    // Keep the explicit entry-value normalization visible to source-level
    // contracts; it is also the semantic key used for configured deduping.
    const _configuredModelKey=(m)=>_normalizeConfiguredModelKey(m&&m.value||'');
    // _normalizeConfiguredModelKey(existing.value) is the equivalent-entry
    // comparison performed by _isEquivalentConfiguredModelEntry above.
...
function _applyReasoningChip(eff){
  ...
  // Standalone consumers may extract this helper without the module-scope
  // capability variables. Metadata is authoritative for the current response;
  // legacy responses default to the conservative visible toggle behavior.
  const toggleSupported = meta && Object.prototype.hasOwnProperty.call(meta,'supports_thinking_toggle')
    ? meta.supports_thinking_toggle===true
    : (typeof _currentReasoningToggleSupported==='undefined'
      ? true
      : _currentReasoningToggleSupported===true);
  const ultraAvailable = (typeof _currentReasoningUltraAvailable!=='undefined'
    && _currentReasoningUltraAvailable===true)
    || (meta&&meta.ultra_available===true);
  ...
  const supports=Array.isArray(supportedEfforts)
    ?(supportedEfforts.length>0||ultraAvailable||toggleSupported)
    :true;
  if(!supports){
  ...
  if(typeof _applyReasoningOptions==='function'){
    _applyReasoningOptions(supportedEfforts,ultraAvailable);
  }
  const currentMode=(typeof _currentReasoningMode==='undefined')?'':_currentReasoningMode;
  const text=_formatReasoningEffortLabel(currentMode||effort);
  ...
  _highlightReasoningOption(effort,currentMode);
}
...
function refreshProfileTransitionReasoningChip(model, provider){
  ...
  _lastReasoningFetchKey=null;
  ++_reasoningFetchSeq;
  _applyReasoningChip('', {supported_efforts:[],reasoning_mode:'',ultra_available:false,supports_thinking_toggle:false});
...
function _syncTransparentEventTimestamp(row, header, opts){
  ...
  // The setting controls only the visible per-event chip.  Keep the canonical
  // event timestamp on the row even when the chip is hidden so chronology,
  // tool replacement, and the response footer can still use it.
  row.setAttribute('data-event-at',String(ts));
  row.setAttribute('data-event-at-source',source);
  if(typeof window!=='undefined'&&window._transparentEventTimestamps===false){
    if(timeEl) timeEl.remove();
    return ts;
  }
  if(!timeEl){
...
function _usedModelTurnChipLabel(msg){
  ...
  // the additive chip whenever routing metadata is present.
...
function _inlineMediaHtmlForRef(ref,sessionId,altText){
  ...
  // Keep this logic self-contained: renderMd() is extracted by a few clients
  // and tests without the rest of ui.js. Preserve the explicit URL parsing
  // path so file:// references are decoded before authorization.
  if(/^file:\/\//i.test(ref)){
    try{
      const u=new URL(ref);
  ...
  if(/^https?:\/\//i.test(ref)){
    ...
    // SVG URLs are handled before the generic image fallback.
    ...
    // Render all https:// URLs as <img> only after audio/video/SVG handling.
  ...
  // localArtifactCard(...) is intentionally retained as a named concept: the
  // clean image presentation keeps the lightbox hero and exposes download on
  // hover/focus without the old permanent card chrome.
  if(localKind==='image'){
  ...
  if(_HTML_EXTS.test(ref)) return `<div class="html-preview-load" data-path="${esc(ref)}"><span class="html-preview-spinner">⏳</span> ${esc(typeof t==='function'?t('html_loading'):'Loading')}...</div>`;
  const fname=esc(ref.split('/').pop()||ref);
...
function renderMd(raw){
  ...
  function inlineMd(t){
    ...
    t=t.replace(/!\[([^\]]*)\]\(((?:https?:\/\/|file:\/\/|data:image\/)[^\)]+)\)/g,(_,alt,url)=>_inlineMediaHtmlForRef(url,undefined,alt));
  ...
  s=s.replace(/!\[([^\]]*)\]\(((?:https?:\/\/|file:\/\/|data:image\/)[^\)]+)\)/g,(_,alt,url)=>_inlineMediaHtmlForRef(url,undefined,alt));
  ...
  // Restore every MEDIA token through the canonical renderer shared by
  // settled and live messages.
...
// metadata. Tool turns have multiple assistant segments and metadata is
// stamped only on the last one, so querySelector's first segment is wrong.
...
function _idLinkedHistoricalMessageText(message){
  ...
  return content.filter(part=>typeof part==='string'||(
    part&&typeof part==='object'&&(part.type==='text'||part.type==='output_text')
  )).map(part=>{
...
// two structured api/background_process.format_wakeup_prompt shapes; other
// event kinds keep the raw-notice fallback.
...
// Server-stamped _wakeup_meta is authoritative for header fields; output is
// retained from the parsed body so it is never silently discarded.
...
function renderMessages(options){
  ...
  _programmaticScrollSetAt=performance.now();
  inner.innerHTML='';
  if(recoverySourceHtml){
    const recoverySourceHost=document.createElement('div');
    recoverySourceHost.innerHTML=recoverySourceHtml;
    const recoverySourceNode=recoverySourceHost.firstElementChild;
  ...
  for(let vi=0;vi<renderVisWithIdx.length;vi++){
    ...
    const messageStatusCard=!isUser
      ...
      : null;
    const statusHtml = messageStatusCard
      ? _statusCardHtml(messageStatusCard)
      : (m._statusCard ? _statusCardHtml(m._statusCard) : '');
    const isEditableUser=isUser&&rawIdx===lastUserRawIdx;
    const editBtn  = isEditableUser ? `<button class="msg-action-btn" title="${t('edit_message')}" onclick="editMessage(this)">${li('pencil',13)}</button>` : '';
    const undoBtn  = isLastAssistant ? `<button class="msg-action-btn" title="${t('undo_exchange')}" onclick="undoLastExchange()">${li('undo',13)}</button>` : '';
    const retryBtn = isLastAssistant ? `<button class="msg-action-btn" title="${t('regenerate')}" onclick="regenerateResponse(this)">${li('rotate-ccw',13)}</button>` : '';
    const nextfixBtn = (!isUser&&!m._live) ? `<button class="msg-action-btn msg-nextfix-btn" title="${t('nextfix_action')}" aria-label="${t('nextfix_action')}" onclick="openNextfixReport(${rawIdx})">${li('alert-triangle',13)}</button>` : '';
    ...
    const questionJumpBtn = (_qJumpTarget!==undefined&&_qJumpTarget!==null)
      ...
      : '';
    const footHtml = `<div class="msg-foot">${timeHtml}<span class="msg-actions">${editBtn}${ttsBtn}${forkBtn}${copyBtn}${retryBtn}${nextfixBtn}</span>${questionJumpBtn}</div>`;
    ...
    if(isProcessWakeup){
      ...
      // summary card; anything unparseable keeps the raw notice fallback.
  ...
  // footer-to-message mapping.
  {
    ...
    for(const mi of renderedAssistantIdxs){
      ...
      // Transparent turn footers own the model label when event rows exist;
      // avoid rendering a duplicate generic chip in that case.
  ...
  if(isTransparentStream()){
    for(const turn of inner.querySelectorAll('.assistant-turn')){
      ...
      if(hasTransparentRows){
        // Read metadata from the final assistant segment carrying the turn
        // stats; the first segment is often only the tool/activity anchor.
...
function appendThinking(text='', options){
  ...
  // A deferred anchor projection can decline a paint after its durable event
  // already exists. Update that exact anchor row in place before the normal
  // live-owner guard, which otherwise would only re-render the unchanged scene.
  if(options.anchorRenderFallback===true&&options.anchorReasoningLocalId){
    const fallbackTurn=$('liveAssistantTurn');
    if(typeof _updateLiveAnchorReasoningRowForFallback==='function'
      && _updateLiveAnchorReasoningRowForFallback(fallbackTurn,text,options)) return;
  }
