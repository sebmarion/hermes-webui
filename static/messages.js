...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _applyToAnchor(sourceEventType, rawEventData, sseEvent, outcome, options){
    ...
    try{
      ...
      if(typeof _adoptStructuredTerminalStateIntoAnchor==='function'){
      ...
      _lastAnchorLiveSceneRendered=shouldRender&&!!_renderAnchorLiveScene();
      if(outcome&&typeof outcome==='object') outcome.rendered=!!_lastAnchorLiveSceneRendered;
      return result;
    }catch(err){
      _lastAnchorLiveSceneRendered=false;
  ...
  function _anchorSceneSettleLiveRunningRow(row, hasSettledThinking){
    ...
    if(row.role==='tool'&&row.tool&&typeof row.tool==='object') sealed.tool={...row.tool,done:true};
    return sealed;
  }
  ...
  function _completeSettledAnchorSceneForTurn(messages, lastAsstIndex, projectedScene){
    ...
    // Final-segment eligibility must use the live projection's chronology.
  ...
  function _anchorProseIncrementalNode(key, text, options){
    ...
    try{
      ...
      if(!st){
        ...
        if(typeof _smdBindParserIdentity === 'function') _smdBindParserIdentity(renderer,st.parser,body);
      ...
      if(finalize) _finalizeAnchorProseIncrementalNode(st);
  ...
  function _clearAnchorProseIncrementalNode(){
    ...
    for(const st of _anchorProseSmdCache.values()){
      if(!st||!st.parser) continue;
      if(typeof _smdMediaTailFlush === 'function') _smdMediaTailFlush(st.parser);
      if(typeof _smdMediaTailClear === 'function') _smdMediaTailClear(st.parser);
      const body=st.node&&st.node.querySelector&&st.node.querySelector('.msg-body');
      if(typeof _smdClearParserIdentity === 'function') _smdClearParserIdentity(body,st.parser);
  ...
  // ui.js owns the image-only data URI policy. Keep this streaming hook tied to
  // that predicate so live and settled rendering cannot drift apart.
  ...
  function _streamFadeRenderer(el){
    ...
    const parserFor=(data)=>_smdParserKey(data,el);
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('approval',e=>{
      ...
      const pendingCount=Math.max(1,Number(d.pending_count)||1);
      showApprovalForSession(activeSid, d, pendingCount);
    ...
    source.addEventListener('done',e=>{
      ...
      const _finishDone=()=>{
        ...
        // Source contract retained: if(!isSessionViewed && typeof _markSessionCompletionUnread==='function')
