...
# ``server`` is the stable public entrypoint even though the implementation
# lives in this split-out module.  Keep the logger name stable for operators
# and for integrations that capture the historical server logger.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_OPTIONS(self) -> None:
        ...
        if not _startup_request_allowed(getattr(self, "command", "OPTIONS"), parsed.path):
            return _deny_startup_fenced_request(self)
...
def _deferred_startup_steps():
    ...
    # Preserve the public ``server.<callable>`` identity after the runtime was
    # split into ``server_runtime.py``.  The release manifest names these
    # operations by their stable entrypoint, and callers inspect the callable
    # metadata when validating that mapping.
    for mutator in mutators_by_operation.values():
        if callable(mutator) and mutator.__module__ == __name__:
            mutator.__module__ = "server"
