...
def _path_stat_cache_key(path):
    ...
    try:
        ...
        return (
            _stat(resolved),
            _stat(resolved.with_name(resolved.name + "-wal")),
            _stat(resolved.with_name(resolved.name + "-shm")),
        )
    except OSError:
        return None
    ...
    *,
    ...
    try:
        with closing(open_state_db_readonly(db_path)) as conn:
            ...
            # Defensive row cap (backstop only). None preserves the existing
            # full-history read for model-context callers.
            limit_clause = ""
            if limit is not None:
                ...
                if limit_int is not None:
                    # Keep the newest rows under the cap, then restore the
                    # reader's oldest-first ordering for callers.
